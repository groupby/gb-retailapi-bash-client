...
ADD gb-retailapi-client /usr/bin/gb-retailapi-client
ADD _gb-retailapi-client /usr/local/share/zsh/site-functions/_gb-retailapi-client
ADD gb-retailapi-client.bash-completion /etc/bash-completion.d/gb-retailapi-client
RUN chmod 755 /usr/bin/gb-retailapi-client
...
RUN echo '\n\
. /etc/bash_completion\n\
source /etc/bash-completion.d/gb-retailapi-client\n\
' >> ~/.bashrc

#
# Setup prompt
#
RUN echo 'export PS1="[GroupBy Retail] \$ "' >> ~/.bashrc
RUN echo 'export PROMPT="[GroupBy Retail] \$ "' >> ~/.zshrc
...
line REST client for $(tput setaf 6)GroupBy Retail$(tput sgr0).\n\
\n\
